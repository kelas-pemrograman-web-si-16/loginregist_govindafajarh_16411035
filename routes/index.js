...
router.post('/login', function(req, res, next) {
    ...
    if (req.body.username == '' || req.body.password == '') {
        ...
        User.find({ //fungsi di mongo db| setelah di isi
            username: req.body.username,
            password: password
        }, function(err, user) {
            if (err) throw err

            if (user.length > 0) { //untuk mengecek
            ...
            } else { //ketika akun salah akan muncul peringatan
...
//view registrasi untuk fungsi pengisisan registrasi
...
router.post('/regis', function(req, res, next) {
    session_store = req.session
    var password = crypto.createHmac('sha256', secret) //sha256 untuk encrypt password
        .update(req.body.password)
        .digest('hex')
    ...
    if (req.body.username == '' || req.body.password == '' || req.body.email == ''
        ...
        User.find({username:req.body.username}, function (err, user){ // untuk mengecek apakah username sudah di pakai atau belum
            if (user.length == 0)
            {
                var member = new User({
                    username: req.body.username,
                    ...
                    admin: false, //untuk menjadi member
                });
                ...
                res.redirect('/regis') //untuk redirek ke halaman login form
            }
        });
    }
})
...
//logout
router.get('/logout', function(req, res){ //logout untuk keluar dari halaman dashboard
